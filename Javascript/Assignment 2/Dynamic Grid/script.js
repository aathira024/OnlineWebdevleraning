...
var productList = [
    ...
    productList.forEach(function(singleCards) {
        htmlCode =
        htmlCode +
        `
        ...
        <img
          src="${singleCards.photos}"
          lt="${
            singleCards.imageAlt
          }">
        />
        <div class="card-text">
          <h3>${singleCards.name}</h3>
          <h4>${singleCards.brand}</h4>
          <h5>${singleCards.price}</h5>
        </div>
        </div>
        `;

    });
    const productcontainer = document.querySelector(".all-cloth-cards");
