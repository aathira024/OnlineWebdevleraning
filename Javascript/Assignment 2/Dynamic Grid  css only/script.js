var productList = [
 ...
 function createTemplate(item){
   const { id, name, preview, photos,description,size,isAccessory,brand,price } = item;
   const template = `
   <div class="card" data-id=${id}>
        ...
        />
         <div class="card-text">
           <h3 class="cardmain-text">${name}
           </h3>
           <h4 class="cardsubtext">${brand}</h4>
          <h5 class="pricetext">${price}</h5>
        </div>
       </div>
       `;
    ...
    //   $.get(function(data) {
    //     const { title, creator, image, id, date, description } = data;
